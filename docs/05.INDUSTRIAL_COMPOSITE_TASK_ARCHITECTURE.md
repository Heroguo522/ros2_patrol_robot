...
| D9 | MQTT topic 复用现有建议 | 继续走现有 `TaskReport` / events 链路，不单独设计新 topic |
| D10 | `feature-add-sim-task` 已推送远端 | 本文档基于该分支整理 |
...
```text
ReportSkill -> /robot/task_report -> robot_gateway_node -> MQTT events
```
...
ros2 topic echo /robot/task_report
...
1. `screw_drive` 返回 `SUCCEEDED` 但 payload 中记录工位失败，这个语义是否符合后续云端统计？
2. `station_result` 枚举是否采用：`success / skipped / failed / partial_failed`？
3. 工件状态第一版是否只保留：`ready_for_screw / not_ready / missing / unknown`？
4. `ScrewDrivingSkill` 是否需要在每个工位开始/结束时额外发布轻量事件，还是仅依赖 `report`？
5. 多工位任务是否需要最终总报告，汇总全部 `station_results`？
6. `mock_arm_fail_station`、`mock_tool_fail_station` 这类固定失败参数是否足够演示？
7. 是否接受第一版 `ReportSkill` 承载复合任务 payload，而不是新增专用 `CompositeTaskReport.msg`？
8. 可视化机械臂是否作为本任务实现目标，还是作为 best effort 附加项？
9. 未来 ROS Action 是定义在 `patrol_interfaces/action/ScrewDrive.action`，还是单独机械臂/工具包中？
10. 是否需要在任务 DSL 中支持 `station_group`，减少多工位重复 YAML？

---

## 20. 仍需澄清的问题

1. “典型工位”第一版是否固定为 4 颗螺丝、目标扭矩 1.2 Nm？
2. 工件识别 mock 的默认结果应始终为 `ready_for_screw`，还是通过参数指定某工位 `not_ready`？
3. 当 `capture_image` 失败时，是否仍允许 `detect_workpiece` 使用 mock 默认图像继续？
4. `screw_drive` 工位失败后，是否需要设置 `/robot/status.fault_code`，还是只在任务报告中体现？
5. 工位汇总结果是否需要包含图片路径，便于云端追溯？
6. 是否需要将 `station_results` 在任务结束时再统一上报一次总报告？
7. 轻量可视化如果改动 URDF，是否允许只显示静态机械臂，不显示动作？
8. 如果后续接 ROS Action，是否希望第一版代码结构提前按 action client/server 拆分目录？
